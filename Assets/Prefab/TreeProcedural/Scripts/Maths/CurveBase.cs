...
namespace Procedural {
    ...
    public abstract class CurveBase {
        ...
        protected virtual Vector3 GetTangent(float t) {
            ...
            var point1 = GetPoint(t1);
            var point2 = GetPoint(t2);
        ...
        public List<FrenetSerret> ComputeFrenetSerrets (int segments, bool closed = false) {
			...
			return ComputeFrenetSerrets(segments, normal, binormal, closed);
        }

		public List<FrenetSerret> ComputeFrenetSerrets(int segments, Vector3 normal, Vector3 binormal, bool closed = false) {
            ...
            for (int i = 1; i <= segments; i++) {
                ...
                if (axis.magnitude > float.Epsilon) {
                    ...
                    eps = Mathf.Acos(Mathf.Clamp(dot, -1f, 1f));
                    normals[i] = Quaternion.AngleAxis(eps * Mathf.Rad2Deg, axis) * normals[i];
                }
                binormals[i] = Vector3.Cross(tangents[i], normals[i]).normalized;
            }

            if (closed) {
                eps = Mathf.Acos(Mathf.Clamp(Vector3.Dot(normals[0], normals[segments]), -1f, 1f));
                eps /= segments;
                ...
                for (int i = 1; i <= segments; i++) {
                    normals[i] = (Quaternion.AngleAxis(Mathf.Deg2Rad * eps * i, tangents[i]) * normals[i]);
                    binormals[i] = Vector3.Cross(tangents[i], normals[i]);
                }
            }

            var frames = new List<FrenetSerret>();
            int n = tangents.Length;
            for(int i = 0; i < n; i++) {
                var frame = new FrenetSerret(tangents[i], normals[i], binormals[i]);
                frames.Add(frame);
            }
            return frames;
		}
